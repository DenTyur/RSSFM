use super::{space::Xspace4D, wave_function::WaveFunction4D};
...
#[derive(Debug, Clone)]
pub struct DoubleIonizProb2e2d {
    pub r_surf: Vec<F>,
    pub ioniz_prob: Vec<Vec<F>>,
    pub t: Array1<F>,
}

impl DoubleIonizProb2e2d {
    ...
    fn calculate_ionization_probability(&self, wf: &WaveFunction4D, r_sq: F, dv: F) -> F {
        let x0 = wf.x.grid[0].clone();
        let x1 = wf.x.grid[1].clone();
        let x2 = wf.x.grid[2].clone();
        let x3 = wf.x.grid[3].clone();

        // Маски для 1 и второго электронов
        let calculate_mask = |x: &Array1<F>, y: &Array1<F>, r_sq: F| {
            Array2::from_shape_fn((x.len(), y.len()), |(i, j)| {
                x[i] * x[i] + y[j] * y[j] > r_sq
            })
        };

        let mask1 = calculate_mask(&x0, &x1, r_sq);
        let mask2 = calculate_mask(&x2, &x3, r_sq);
                ...
                for i1 in 0..x1.len() {
                    ...
                    for i2 in 0..x2.len() {
                        for i3 in 0..x3.len() {
                            ...
                            if mask2[[i2, i3]] {
                                local_sum += wf.psi[[i0, i1, i2, i3]].norm_sqr();
                            }
                        }
                    }
                }
    ...
    pub fn add(&mut self, wf: &WaveFunction4D) {
        let dv = wf.x.dx[0] * wf.x.dx[1] * wf.x.dx[2] * wf.x.dx[3];
