...
impl WaveFunction<1> for WaveFunction1D {
    ...
    fn sparse(&mut self, sparse_step: isize) {
        let psi = self.psi.slice(s![..;sparse_step]);
        ...
        self.x = Xspace1D {
            x0: [x_grid0_sparsed[0]],
            dx: [x_grid0_sparsed[1] - x_grid0_sparsed[0]],
            n: [x_grid0_sparsed.len()],
            grid: [x_grid0_sparsed],
        };

        self.p = Pspace1D {
            p0: [p_grid0_sparsed[0]],
            dp: [p_grid0_sparsed[1] - p_grid0_sparsed[0]],
            n: [p_grid0_sparsed.len()],
            grid: [p_grid0_sparsed],
        };
    }
