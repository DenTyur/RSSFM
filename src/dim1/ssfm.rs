...
pub struct SSFM1D<'a, G, AP, AB>
where
    AP: Fn([F; 1]) -> F + Send + Sync,
    AB: Fn([F; 1]) -> C + Send + Sync,
    G: GaugedEvolutionSSFM<1, AP, AB, WF = WaveFunction1D>,
{
...
impl<'a, G, AP, AB> SSFM1D<'a, G, AP, AB>
where
    AP: Fn([F; 1]) -> F + Send + Sync,
    AB: Fn([F; 1]) -> C + Send + Sync,
    G: GaugedEvolutionSSFM<1, AP, AB, WF = WaveFunction1D>,
...
// Исправленная реализация типажа SSFM
impl<'a, G, AP, AB> SSFM for SSFM1D<'a, G, AP, AB>
where
    AP: Fn([F; 1]) -> F + Send + Sync,
    AB: Fn([F; 1]) -> C + Send + Sync,
    G: GaugedEvolutionSSFM<1, AP, AB, WF = WaveFunction1D>,
