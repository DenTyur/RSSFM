...
use crate::dim3::{space::Xspace3D, wave_function::WaveFunction3D};
...
impl TimeFFT {
    ...
    pub fn new(t: Tspace, point: [F; 3], x: &Xspace3D) -> Self {
        ...
        Self {
            t,
            point,
            ind_point: [ind_point_x0, ind_point_x1, ind_point_x2],
            psi_in_point,
            energy,
            psi_fft,
        }
    }
    pub fn add_psi_in_point(&mut self, wf: &WaveFunction3D) {
        self.psi_in_point
            .push(wf.psi[(self.ind_point[0], self.ind_point[1], self.ind_point[2])]);
    }
